...
import {ExternalLink, Facebook, HeartHandshake as Handshake, Instagram, Twitter as TwitterIcon} from 'lucide-react';
			...
			<a href={sponsor.website_url || '#'} target="_blank" rel="noopener noreferrer"
			   className="block w-full flex flex-col flex-grow">
				<div className="h-24 w-full flex items-center justify-center mb-3 bg-gray-700/30 rounded flex-shrink-0">
					{sponsor.logo_url ? (
						<img src={sponsor.logo_url} alt={`Logo de ${sponsor.name}`}
						     className="max-h-20 w-auto object-contain filter grayscale group-hover:grayscale-0 transition-all duration-300"/>
					) : (
						<div className="w-full h-full flex items-center justify-center text-muted-foreground text-sm">Logo no
							disponible</div>
					)}
				</div>
				<p
					className="text-sm font-semibold text-muted-foreground group-hover:text-primary transition-colors mt-auto">{sponsor.name}</p>
			</a>
			<div className="flex space-x-3 mt-3 flex-shrink-0">
				{sponsor.website_url && (
					<a href={sponsor.website_url} target="_blank" rel="noopener noreferrer" aria-label={`${sponsor.name} website`}
				...
				{sponsor.twitter_url && (
					<a href={sponsor.twitter_url} target="_blank" rel="noopener noreferrer" aria-label={`${sponsor.name} Twitter`}
					   className="text-muted-foreground/70 hover:text-primary transition-colors">
						<TwitterIcon size={18}/>
					</a>
				)}
...
const SponsorsSection = () => {
	...
	const fetchSponsors = useCallback(async () => {
		...
		console.log('Sponsors fetched:', sponsorsData);
	}, []);
	...
	return (
		<motion.section id="sponsors" variants={fadeIn(0.4)} initial="hidden" animate="visible" className="py-12">
			<h2 className="text-4xl font-semibold text-center mb-12  flex items-center justify-center">
				<Handshake className="mr-3 h-10 w-10 text-primary"/> Nos Auspician
			</h2>
			{sponsors.length > 0 ? (
				<motion.div
			...
			<div className="text-center mt-10">
				<p className="text-muted-foreground">
					...
					<Button variant="link" asChild className="text-primary p-0 h-auto ml-1">
						<a target="_blank" href={`mailto:opensaltabjj@gmail.com?subject=Quiero ser patrocinador&body=Hola, me gustaría ser patrocinador de Open Salta BJJ 2025.`}>
							Contáctanos
						</a>
					</Button>
				</p>
